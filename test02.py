...
for rankitem in ranklist['data']:
    if rankitem['name'] not in rankdict:
        rankdict[rankitem['name']] = rankitem['confirm']
#print(rankdict)
...
for rankitem in rankdict:
    ...
    if i % 8 == 0:
        ...
        plt.title('新冠累计确诊', fontproperties=myfont, fontsize=16)
        plt.xlim((0, len(x)))
    ...
    for dataitem in datalist['data']:
        if dataitem['date'].replace('.', '') not in confirmdict:
            confirmdict[dataitem['date'].replace('.', '')] = dataitem['confirm']
    #print(confirmdict)
...
if flag == True:
    ...
    ltext = leg.get_texts()
    plt.setp(ltext, fontsize='16')
    plt.savefig('./test02.' + str((i + 7) // 8) + '.png')
